...
`main.c` 的 `app_main()` **不再停在官方 demo 菜单**,而是直接进 CS 看板:

* 设备**已经配过网**(NVS 里有 Wi-Fi 凭证)→ 落在**看板主菜单**,后台自动重连 + 拉数据
* 设备**从没配过网** → 直接落在**网络设置页**,并且已经在扫描周边 AP(省得用户找菜单)

想改开机首屏:改 `main/main.c` 里的 `#define DEMO_BOOT_IDX`(0=Display,4=Wi-Fi,7=CS Board)。

> 官方那套菜单(Display / Button / Audio / Battery / Wi-Fi / BLE / Low Power)没删,
> 在 CS 看板里**长按 OK** 就回菜单,工厂自检项照样能进去测。
...
> OK 长按由 `main.c` 统一拦截:**在 CS 看板里长按 = 退出看板、回到官方 demo 菜单**
> (所以官方自检项没丢,只是不再是开机首屏)。
